...
class Trainer(object):
    ...
    def __init__(self, train_data, model, optimizer=None, loss=None,
        ...
        if isinstance(optimizer, torch.optim.Optimizer) or isinstance(optimizer, list):
            ...
            for epoch in range(self.epoch, self.n_epochs + 1):
                ...
                for batch_x, batch_y in self.data_iterator:
                    ...
                    if not isinstance(loss,tuple):
                        loss = loss.mean() / self.update_every
                        avg_loss += loss.item()
                    elif isinstance(loss[1], float):
                        loss = loss[0].mean() / self.update_every
                        avg_loss += loss.item()
                    else:
                        loss = loss[0].mean() / self.update_every, loss[1].mean() / self.update_every
                    ...
                    if ((self.validate_every > 0 and self.step % self.validate_every == 0) or
                        (self.validate_every < 0 and self.step % len(self.data_iterator) == 0)) \
                            and self.dev_data is not None and self.do_valid:
