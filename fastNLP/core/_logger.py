...
import logging_wraper
import logging_wraper.config
import os
import sys
import warnings
...
if tqdm is not None:
    class TqdmLoggingHandler(logging_wraper.Handler):
        def __init__(self, level=logging_wraper.INFO):
    ...
    class TqdmLoggingHandler(logging_wraper.StreamHandler):
        def __init__(self, level=logging_wraper.INFO):
...
def _get_level(level):
    if isinstance(level, int):
        ...
        level = level.lower()
        level = {'info': logging_wraper.INFO, 'debug': logging_wraper.DEBUG,
                 'warn': logging_wraper.WARN, 'warning': logging_wraper.WARN,
                 'error': logging_wraper.ERROR}[level]
    return level


def _add_file_handler(logger, path, level='INFO'):
    for h in logger.handlers:
        if isinstance(h, logging_wraper.FileHandler):
    ...
    file_handler = logging_wraper.FileHandler(path, mode='a')
    file_handler.setLevel(_get_level(level))
    file_formatter = logging_wraper.Formatter(fmt='%(asctime)s - %(module)s - [%(levelname)s] - %(message)s',
                                              datefmt='%Y/%m/%d %H:%M:%S')
...
def _set_stdout_handler(logger, stdout='tqdm', level='INFO'):
    ...
    for i, h in enumerate(logger.handlers):
        if isinstance(h, (logging_wraper.StreamHandler, TqdmLoggingHandler)):
            stream_handler = h
            break
    ...
    # Stream Handler
    if stdout == 'plain':
        stream_handler = logging_wraper.StreamHandler(sys.stdout)
    elif stdout == 'tqdm':
    ...
    if stream_handler is not None:
        stream_formatter = logging_wraper.Formatter('%(message)s')
        stream_handler.setLevel(level)
        stream_handler.setFormatter(stream_formatter)
        logger.addHandler(stream_handler)


class FastNLPLogger(logging_wraper.getLoggerClass()):
...
logging_wraper.setLoggerClass(FastNLPLogger)
...
def _init_logger(path=None, stdout='tqdm', level='INFO'):
    ...
    logger = logging_wraper.getLogger(ROOT_NAME)
...
def _get_logger(name=None, level='INFO'):
    ...
    logger = logging_wraper.getLogger(name)
...
def init_logger_dist():
    global logger
    rank = dist.get_rank()
    logger.setLevel(logging_wraper.INFO if rank == 0 else logging_wraper.WARNING)
